...
class PendulumState final : public drake::systems::BasicVector<T> {
 public:
  ...
  PendulumState<T>* DoClone() const final { return new PendulumState; }
  ...
  PendulumState<T> with_theta(const T& theta) const {
  ...
  DRAKE_VECTOR_GEN_NODISCARD
  PendulumState<T> with_thetadot(const T& thetadot) const {
    PendulumState<T> result(*this);
